...
4.  type_info头文件定义了bad_cast异常类型
    ...
    ## assert和NDEBUG
    assert通常用来调试程序，包含在cassert头文件中，输入参数是一个表达式，当结果为true时不做任何处理，为false时终止程序运行并输出报错信息。assert只在没有定义NDEBUG变量时有效，如果手动定义了NDEBUG：`#define NDEBUG`则assert不会有任何作用，在编译时也可以通过指令添加定义：
    ```C++
    g++ -D NDEBUG main.cc 
    ```
    以上指令等效于在程序最前面添加了NDEBUG的定义。<br>
